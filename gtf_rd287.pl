...
";
my $detailedusage = "
The reporting of results is directed by flags. 
...
do {
    ...
    $choice1 = <STDIN>;
    chomp($choice1);
} until ( $choice1 eq 'Y' || $choice1 eq 'N'); # Keep asking until getting 'Y' or 'N'.
if ($choice1 eq 'Y') {
	do {
		print"type the new name.(ending with .gtf)\nThere are the candidate file names:\n\t";
		system ("ls *.gtf");
		$filename=<STDIN>; 
		chomp($filename);			# get the file name of the new inputfile.	
	} until ($filename=~ /\.gtf$/); # Keep asking until get the name of inputfile in right format.
...
if ($opt_n) {
    ...
    foreach ( @{$referenced_names} ) { # Dereference the array and print each element there.
